...
export default {
  ...
  methods: {
    ...
    async getIdsMerchantOnly() {
      ...
      try {
        const docRef = await db.collection('merchant').doc(merchant);
        const doc = await docRef.get();
        if (doc.exists) {
          this.id_merchant_only = doc.data().list;
          console.log(this.id_merchant_only)
        } else {
          alert("Este usuario no esta habilitado para el callcenter");
          // redireccionar
        }
