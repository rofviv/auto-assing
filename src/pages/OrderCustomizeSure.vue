...
export default {
  ...
  data() {
    return {
      ...
      //CAMBIAR
      user_identifier: 'ff96ca9fc18307',
      operator_token: 'ca3686de8b9cd13abcb362e09e494210',
      access_token: '71b8e78b375f8e2135bd642848ac49e4396cbf326034bb203855aab3cfdd50f9',
  ...
  methods: {
    async getAddress() {
      try {
        // CAMBIAR
        const res = await db.collection('address-sure').get();
        res.forEach(doc => {
    ...
    async saveAddressLocal() {
      ...
      try {
        // CAMBIAR
        const res = await db.collection("address-sure").add(auxAddress);
        if (res.id) {
